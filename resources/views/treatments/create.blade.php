<x-admin-layout title="Tratamientos | DentalPlus" :breadcrumbs="[
    ...
    <x-wire-card>
        <form action="{{ route('treatments.store') }}" method="POST">
            @csrf
            <x-wire-input label="Nombre" name="nombre" placeholder="Nombre del tratamiento" value="{{ old('nombre') }}"></x-wire-input>
            <x-wire-input label="Duración (min)" name="duracion" type="number" placeholder="Duración en minutos" value="{{ old('duracion') }}"></x-wire-input>
            <x-wire-input label="Costo" name="costo" type="number" step="0.01" placeholder="Precio" value="{{ old('costo') }}"></x-wire-input>
            <x-wire-input label="Cuidados" name="cuidados" placeholder="Indicaciones" value="{{ old('cuidados') }}"></x-wire-input>
            <div class="mt-4">
