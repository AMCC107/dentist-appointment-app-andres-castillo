<x-admin-layout title="Citas | DentalPlus" :breadcrumbs="[
    ...
    <x-wire-card>
        <form action="{{ route('appointments.store') }}" method="POST">
            @csrf
            <x-wire-input label="Nombre" name="nombre" placeholder="Título de la cita" value="{{ old('nombre') }}"></x-wire-input>

            <x-wire-input label="Fecha y hora" name="fecha_hora" type="datetime-local" placeholder="YYYY-MM-DD HH:MM" value="{{ old('fecha_hora') }}"></x-wire-input>
            ...
            <div class="mt-4">
                ...
                <select id="paciente_id" name="paciente_id" class="w-full rounded border-gray-300 p-2">
                    ...
                    @foreach($patients as $p)
                        <option value="{{ $p->id }}" {{ old('paciente_id') == $p->id ? 'selected' : '' }}>{{ $p->name }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="mt-4">
                ...
                <select id="tratamiento_id" name="tratamiento_id" class="w-full rounded border-gray-300 p-2">
                    ...
                    @foreach($treatments as $t)
                        <option value="{{ $t->id }}" {{ old('tratamiento_id') == $t->id ? 'selected' : '' }}>{{ $t->nombre }}</option>
                    @endforeach
                </select>
            </div>
